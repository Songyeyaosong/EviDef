...
class TEMS2ANet(RotatedBaseDetector):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(TEMS2ANet, self).__init__()

        backbone.pretrained = pretrained
        self.backbone = build_backbone(backbone)
        self.tem = build_neck(tem)
        if neck is not None:
    ...
    def simple_test(self, img, img_meta, rescale=False):
        ...
        # rois: list(indexed by images) of list(indexed by levels)
        align_feat = self.align_conv(x, rois)
        # show_feat = align_feat[0]
        # show_feature_map(show_feat)
        outs = self.odm_head(align_feat)

        # outs = self.single_level_outs(outs, 0)
        # rois = self.single_level_rois(rois, 0)
    ...
    def single_level_outs(self, outs, index):

        single_cls_score = []
        single_bbox_pred = []
        cls_score, bbox_pred = outs
        single_cls_score.append(cls_score[index])
        single_bbox_pred.append(bbox_pred[index])
        return single_cls_score, single_bbox_pred
    
    def single_level_rois(self, rois, index):

        single_rois = []
        single_rois.append(rois[0][index])  # test always only have 1 image
        return [single_rois]
