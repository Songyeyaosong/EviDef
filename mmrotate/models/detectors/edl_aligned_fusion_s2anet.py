...
class EdlAlignedFusionS2ANet(RotatedBaseDetector):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(EdlAlignedFusionS2ANet, self).__init__()
    ...
    def forward_train(self,
        ...
        cls_scores, bbox_preds, fusion_feat = self.fusion_head(align_feat)
        loss_inputs = (cls_scores, bbox_preds) + (gt_bboxes, gt_labels, img_metas)
    ...
    def simple_test(self, img, img_meta, rescale=False):
        ...
        # levels_rois = [torch.stack(roi, dim=0) for roi in zip(*rois)]  # 将rois从按batch排列转换为按level排列
        align_feat = self.align_conv(x, rois)
        cls_score, bbox_pred, fusion_feat = self.fusion_head(align_feat)
