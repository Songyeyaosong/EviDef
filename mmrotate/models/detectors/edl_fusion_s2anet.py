...
class EDLFUSIONS2ANet(RotatedBaseDetector):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(EDLFUSIONS2ANet, self).__init__()
    ...
    def forward_train(self,
        ...
        cls_scores, bbox_preds, fusion_feats  = self.fam_head.fusion_forward(x)  # cls_score, bbox_pred
        ...
        align_feat = self.align_conv(fusion_feats, rois)
        outs = self.odm_head(align_feat)
    ...
    def simple_test(self, img, img_meta, rescale=False):
        ...
        """
        x = self.extract_feat(img)
        cls_scores, bbox_preds, fusion_feats  = self.fam_head.fusion_forward(x)
        rois = self.fam_head.refine_bboxes(cls_scores, bbox_preds)
        # rois: list(indexed by images) of list(indexed by levels)
        # levels_rois = [torch.stack(roi, dim=0) for roi in zip(*rois)]  # 将rois从按batch排列转换为按level排列
        align_feat = self.align_conv(fusion_feats, rois)
        cls_score, bbox_pred = self.odm_head(align_feat)
