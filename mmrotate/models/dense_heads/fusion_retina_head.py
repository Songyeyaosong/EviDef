...
class FusionRetinaHead(EdlFusionAnchorHead):
    ...
    def __init__(self,
        ...
        super(FusionRetinaHead, self).__init__(
            num_classes,
            in_channels,
    ...
    def evidence_fusion_module(self, feat_top, feat_down, cls_score_top, cls_score_down):
        ...
        top_b1, top_b2, top_u = self.get_max_b_u(evidence_top)
        down_b1, down_b2, down_u = self.get_max_b_u(evidence_down)
    ...
    def get_max_b_u(self, evidence):

        evidence = evidence.permute(0, 2, 3, 1)
        batch, h, w = evidence.shape[:3]
        evidence = evidence.reshape(batch, h, w, self.num_anchors, 2)
        e1 = evidence[..., 0]
        e2 = evidence[..., 1]
        s = e1 + e2 + 2
        b1 = e1 / s
        ...
        max_b1, idx = torch.max(b1, dim=-1, keepdim=True)
        max_b2 = torch.gather(b2, dim=-1, index=idx)
        max_u = torch.gather(u, dim=-1, index=idx)
        max_b1, max_b2, max_u = max_b1.permute(0, 3, 1, 2).squeeze(dim=1), max_b2.permute(0, 3, 1, 2).squeeze(dim=1), max_u.permute(0, 3, 1, 2).squeeze(dim=1)

        return max_b1, max_b2, max_u
