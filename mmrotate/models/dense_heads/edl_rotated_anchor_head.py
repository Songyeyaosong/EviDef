...
class EdlRotatedAnchorHead(BaseDenseHead):
    ...
    def __init__(self,
        ...
        super(EdlRotatedAnchorHead, self).__init__(init_cfg)
        self.in_channels = in_channels
        self.num_classes = num_classes
        self.feat_channels = feat_channels
        self.is_obj_score = is_obj_score
        self.use_sigmoid_cls = loss_cls.get('use_sigmoid', True)
        self.need_bg = loss_cls.get('need_bg', False)
        # TODO better way to determine whether sample or not
        self.sampling = loss_cls['type'] not in [
            'FocalLoss', 'GHMC', 'QualityFocalLoss', 'EDLFocalLoss', 'EDLFocalLoss3D'
        ]  # 如果不是focalloss, 那么就需要采样, 是focalloss就不需要了
        if self.use_sigmoid_cls and not self.need_bg:
    ...
    def ps_forward(self, feats, rois):
        """Forward features from the upstream network.
        ...
        Returns:
            tuple: A tuple of classification scores and bbox prediction.
                ...
                - bbox_preds (list[Tensor]): Box energies / deltas for all \
                    ...
                    is num_anchors * 5.
        """
        return multi_apply(self.ps_forward_single, feats, rois)
    
    def conv3d_forward(self, feats):
        """Forward features from the upstream network.
        ...
        Returns:
            tuple: A tuple of classification scores and bbox prediction.
                ...
                - bbox_preds (list[Tensor]): Box energies / deltas for all \
                    ...
                    is num_anchors * 5.
        """
        return multi_apply(self.conv3d_forward_single, feats)
    ...
    def _get_targets_single(self,
        ...
        """
        inside_flags = rotated_anchor_inside_flags(  # 看看这个bbox是不是在图像里面
            flat_anchors, valid_flags, img_meta['img_shape'][:2],
            self.train_cfg.allowed_border)
        if not inside_flags.any():  # 如果一个bbox都不在图像里
            return (None, ) * 7
        # assign gt and sample anchors
        anchors = flat_anchors[inside_flags, :]  # 只取出有效的bbox
    ...
    def get_targets(self,
        ...
        num_level_anchors = [anchors.size(0) for anchors in anchor_list[0]]  # 特征金字塔的层数
        # concat all level anchors to a single tensor
        concat_anchor_list = []  # 因为是算loss, 所以要把一个batch所有的bbox都拿过来
        concat_valid_flag_list = []
        for i in range(num_imgs):  # 把所有的bbox合成为一个tensor
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
                    bbox_targets, bbox_weights, num_total_samples, current_epoch):
        ...
        if len(cls_score.shape) == 4:
            cls_score = cls_score.permute(0, 2, 3,
                                      1).reshape(-1, 2 if self.is_obj_score else self.cls_out_channels * 2)
        elif len(cls_score.shape) == 5:
            cls_score = cls_score.permute(0,3,4,2,
                                      1).reshape(-1, self.cls_out_channels, 2)

        if current_epoch:
            loss_cls = self.loss_cls(
                cls_score, labels, label_weights, avg_factor=num_total_samples, current_epoch=current_epoch)
        else:
            loss_cls = self.loss_cls(
    ...
    def loss(self,
        ...
        label_channels = self.cls_out_channels if self.use_sigmoid_cls else 1  # 这个label_channels应该指的是gt的标注是onehot还是index
        cls_reg_targets = self.get_targets(  # 返回每个bbox分配好的label和gtbbox, bbox正样本的权重为1, 负样本和背景的权重就为0
            anchor_list,
            valid_flag_list,
            gt_bboxes,
            img_metas,
