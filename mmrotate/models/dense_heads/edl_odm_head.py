...
from ..utils import ORConv2d, RotationInvariantPooling, RSPool, RSConv
from .edl_rotated_retina_head import EdlRotatedRetinaHead


@ROTATED_HEADS.register_module()
class EdlOdmHead(EdlRotatedRetinaHead):
    ...
    def __init__(self,
        ...
        super(EdlOdmHead, self).__init__(
            num_classes,
            in_channels,
            stacked_convs=2,
    ...
    def _init_layers(self):
        ...
        self.rs_pool = RSPool(self.feat_channels, 8)
        self.rs_conv = RSConv(self.feat_channels, 8)
        self.cls_convs = nn.ModuleList()
        self.reg_convs = nn.ModuleList()
        self.rs_reg_convs = nn.ModuleList()
        self.cls_convs3d = nn.ModuleList()
        for i in range(self.stacked_convs):
            chn = int(self.feat_channels / 8) if i == 0 else self.feat_channels
            in_channels_3d = 1 if i == 0 else self.feat_channels / 8
            out_channels_3d = self.feat_channels / 8
            ...
            self.rs_reg_convs.append(
                ConvModule(
                    ...
                    conv_cfg=self.conv_cfg,
                    norm_cfg=self.norm_cfg))
            self.cls_convs3d.append(
                nn.Sequential(
                    nn.Conv3d(int(in_channels_3d), int(out_channels_3d), kernel_size=1),
                    nn.ReLU()
                )
            )
        ...
        self.cls_conv_before3d = nn.Conv2d(int(self.feat_channels / 8), int(self.num_classes + 1), kernel_size=3, padding=1)
        self.edl_odm_cls = nn.Conv2d(
            self.feat_channels,
            self.num_anchors * self.cls_out_channels * 2,
            3,
            padding=1)
        self.rs_odm_reg = nn.Conv2d(
            int(self.feat_channels / 8), self.num_anchors * 5, 3, padding=1)
        self.edl_odm_cls3d = nn.Conv3d(
            int(self.feat_channels / 8),
            2,
            kernel_size=1
        )
    ...
    def forward_single(self, x):
        ...
        cls_score = self.edl_odm_cls(cls_feat)
        bbox_pred = self.odm_reg(reg_feat)
        return cls_score, bbox_pred
    
    def ps_forward_single(self, x, rois):
        """Forward feature of a single scale level.
        ...
        """
        or_feat = self.or_conv(x)
        reg_feat = self.rs_pool(or_feat, rois)
        cls_feat = self.or_pool(or_feat)
        for cls_conv in self.cls_convs:
            cls_feat = cls_conv(cls_feat)
        for rs_reg_conv in self.rs_reg_convs:
            reg_feat = rs_reg_conv(reg_feat)
        cls_score = self.edl_odm_cls(cls_feat)
        bbox_pred = self.rs_odm_reg(reg_feat)
        return cls_score, bbox_pred
    
    def conv3d_forward_single(self, x):
        """Forward feature of a single scale level.
        ...
        reg_feat = or_feat
        cls_feat = self.or_pool(or_feat)
        cls_feat = self.cls_conv_before3d(cls_feat)
        cls_feat = cls_feat.unsqueeze(1)
        for cls_conv3d in self.cls_convs3d:
            cls_feat = cls_conv3d(cls_feat)
        for reg_conv in self.reg_convs:
            reg_feat = reg_conv(reg_feat)
        cls_score = self.edl_odm_cls3d(cls_feat)
        bbox_pred = self.odm_reg(reg_feat)
        return cls_score, bbox_pred
    ...
    def loss(self,
        ...
        return super(EdlOdmHead, self).loss(
