...
class EdlFusionFamHead(EdlFusionFamAnchorHead):
    ...
    def __init__(self,
        ...
        super(EdlFusionFamHead, self).__init__(
            num_classes,
            in_channels,
    ...
    def evidence_fusion_module(self, feat_top, feat_down, cls_score_top, cls_score_down):
        ...
        # TODO: try fusion weight
        fusion_weight = (TF.resize(top_b1, down_size)[:,None,:,:] * down_b1[:,None,:,:] + \
                       TF.resize(top_b1, down_size)[:,None,:,:] * down_u[:,None,:,:] + \
                        down_b1[:,None,:,:] * TF.resize(top_u, down_size)[:,None,:,:]) / \
                            (1 - c)[:,None,:,:]
        fusion_feat = feat_down * fusion_weight
