...
class UGRotatedAnchorHead(BaseDenseHead):
    ...
    def __init__(self,
        ...
        super(UGRotatedAnchorHead, self).__init__(init_cfg)
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        label_weights = label_weights.permute(0, 2, 3, 1).reshape(-1, self.cls_out_channels)
        cls_score = cls_score.permute(0, 2, 3, 1).reshape(-1, self.cls_out_channels)
        cls_label = cls_score.argmax(dim=1)
        label_weights = torch.gather(label_weights, dim=1, index=cls_label.unsqueeze(1)).squeeze(1)
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, anchors in zip(cls_score_list,
            ...
            # this is for mami merge
            # original_shape = anchors.shape
            # repeated_anchors = anchors.unsqueeze(1).repeat(1, 2, 1)
            # anchors = repeated_anchors.reshape(original_shape[0] * 2, -1)
