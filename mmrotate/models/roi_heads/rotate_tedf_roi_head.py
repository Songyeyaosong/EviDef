...
class RotatedTedfRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        super(RotatedTedfRoIHead, self).__init__(init_cfg)
    ...
    def _fine_bbox_forward(self, bbox_feats):

        if self.with_shared_head:
            bbox_feats = self.shared_head(bbox_feats)
        cls_score = self.bbox_head.fine_forward(bbox_feats)

        bbox_results = dict(
            fine_cls_score=cls_score, bbox_feats=bbox_feats)
        return bbox_results
    ...
    def simple_test(self, x, te_d3, te_d5, proposal_list, img_metas, rescale=False):
        ...
        det_bboxes, det_labels = self.simple_test_bboxes(
            x, te_d3, te_d5, img_metas, proposal_list, self.test_cfg, rescale=rescale)
