...
class RotatedTedfConvFCBBoxHead(RotatedTedfBBoxHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(RotatedTedfConvFCBBoxHead, self).__init__(
        ...
        self.ted3_convs, self.ted3_fcs, _ = \
            self._add_conv_fc_branch(
                self.num_shared_convs, self.num_shared_fcs, 256,
                True)
        
        self.ted5_convs, self.ted5_fcs, _ = \
            self._add_conv_fc_branch(
                self.num_shared_convs, self.num_shared_fcs, 256,
                True)
        ...
        if self.with_cls:
            ...
            self.ted5_fc_cls = build_linear_layer(
                self.cls_predictor_cfg,
                in_features=self.cls_last_dim,
                out_features=cls_channels)
        ...
        if self.with_reg:
            out_dim_reg = (5 if self.reg_class_agnostic else 5 *
                           self.num_classes)
            self.tedf_fc_reg = build_linear_layer(
                self.reg_predictor_cfg,
                in_features=self.reg_last_dim,
                out_features=out_dim_reg)
            self.fine_fc_reg = build_linear_layer(
                self.reg_predictor_cfg,
                in_features=self.reg_last_dim,
                out_features=out_dim_reg)
    ...
    def w_forward(self, x):

        w = self.w_conv(x)
        w = torch.sigmoid(w)
        return w
    
    def ted5_forward(self, x):
        ...
        cls_pred = self.ted5_fc_cls(x_cls) if self.with_cls else None
        return cls_pred
    ...
    def ted3_forward(self, x):
        ...
        cls_pred = self.ted3_fc_cls(x_cls) if self.with_cls else None
        return cls_pred
...
@ROTATED_HEADS.register_module()
class RotatedTedfShared2FCBBoxHead(RotatedTedfConvFCBBoxHead):
    """Shared2FC RBBox head."""

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(RotatedTedfShared2FCBBoxHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
...
@ROTATED_HEADS.register_module()
class RotatedTedfKFIoUShared2FCBBoxHead(RotatedTedfConvFCBBoxHead):
    """KFIoU RoI head."""

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(RotatedTedfKFIoUShared2FCBBoxHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
