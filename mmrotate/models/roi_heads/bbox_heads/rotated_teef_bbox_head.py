...
from mmrotate.core import build_bbox_coder, multiclass_nms_rotated, edl_multiclass_nms_rotated
from ...builder import ROTATED_HEADS, build_loss


@ROTATED_HEADS.register_module()
class RotatedTeefBBoxHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(RotatedTeefBBoxHead, self).__init__(init_cfg)
    ...
    @force_fp32(apply_to=('cls_score', 'e_sum', 'bbox_pred'))
    def loss(self,
             cls_score,
             e_sum,
             bbox_pred,
        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                ...
                if isinstance(loss_cls_, dict):
                    ...
                    losses['ted3_loss_cls'] = loss_cls_
                if self.custom_activation:
                    acc_ = self.loss_cls.get_accuracy(cls_score, labels)
                    losses.update(acc_)
                else:
                    losses['ted3_acc'] = accuracy(cls_score, labels)
        if bbox_pred is not None:
            bg_class_ind = self.num_classes
            # 0~self.num_classes-1 are FG, self.num_classes is BG
            pos_inds = (labels >= 0) & (labels < bg_class_ind)
            ...
            if pos_inds.any():
                ...
                if self.reg_class_agnostic:
                    ...
                    pos_bbox_pred = bbox_pred.view(
                        ...
                        5)[pos_inds.type(torch.bool),
                           labels[pos_inds.type(torch.bool)]]
                losses['ted3_loss_bbox'] = self.loss_bbox(
                    pos_bbox_pred,
                    bbox_targets[pos_inds.type(torch.bool)],
                    bbox_weights[pos_inds.type(torch.bool)],
                    avg_factor=bbox_targets.size(0),
                ...
                losses['ted3_loss_bbox'] = bbox_pred[pos_inds].sum()
        return losses

    @force_fp32(apply_to=('cls_score', 'bbox_pred', 'u'))
    def get_bboxes(self,
                   rois,
                   cls_score,
                   bbox_pred,
        ...
        if self.custom_cls_channels:
            ...
            # evidence = F.softplus(cls_score)
            # evidence = self.exp_evidence(cls_score)
            # alpha = evidence + 1
        ...
        if cfg is None:
            ...
            return bboxes, scores, u
        else:
            # det_bboxes, det_labels = multiclass_nms_rotated(
            #     bboxes, scores, cfg.score_thr, cfg.nms, cfg.max_per_img)
            det_bboxes, det_labels = edl_multiclass_nms_rotated(
                bboxes, scores, u, cfg.score_thr, cfg.nms, cfg.max_per_img)
            return det_bboxes, det_labels
        
    def exp_evidence(self, x):
