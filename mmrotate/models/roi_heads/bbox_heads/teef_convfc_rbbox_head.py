...
class RotatedTeefConvFCBBoxHead(RotatedTeefBBoxHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(RotatedTeefConvFCBBoxHead, self).__init__(
        ...
        self.ted3_convs, self.ted3_fcs, _ = \
            self._add_conv_fc_branch(
                self.num_shared_convs, self.num_shared_fcs, self.in_channels,
                True)
        
        self.ted3_e_sum_convs, self.ted3_e_sum_fcs, _ = \
            self._add_conv_fc_branch(
                self.num_shared_convs, self.num_shared_fcs, self.in_channels,
                True)

        self.e_sum_convs, self.e_sum_fcs, _ = \
            self._add_conv_fc_branch(
                self.num_shared_convs, self.num_shared_fcs, self.in_channels,
                True)
        ...
        if self.with_cls:
            ...
            self.ted3_fc_cls = build_linear_layer(
                self.cls_predictor_cfg,
                in_features=self.cls_last_dim,
                out_features=cls_channels)
            self.ted3_e_sum = build_linear_layer(
                self.cls_predictor_cfg,
                in_features=self.cls_last_dim,
                out_features=1)
        ...
        if self.with_reg:
            out_dim_reg = (5 if self.reg_class_agnostic else 5 *
                           self.num_classes)
            self.ted3_fc_reg = build_linear_layer(
    ...
    def ted3_forward(self, x):

        x = x.flatten(1)

        for fc in self.ted3_fcs:
            x = self.relu(fc(x))

        x_cls = x
        x_reg = x

        ted3_cls_score = self.ted3_fc_cls(x_cls)
        ted3_bbox_pred = self.ted3_fc_reg(x_reg)
        return ted3_cls_score, ted3_bbox_pred
    
    def ted3_e_sum_forward(self, x):

        x = x.flatten(1)

        for fc in self.ted3_e_sum_fcs:
            x = self.relu(fc(x))

        x_cls = x
        
        teef_e_sum = self.ted3_e_sum(x_cls)
        return teef_e_sum
    ...
    def e_sum_forward(self, x):
        ...
        e_sum = self.e_sum(x_cls) if self.with_cls else None
        return e_sum


@ROTATED_HEADS.register_module()
class RotatedTeefShared2FCBBoxHead(RotatedTeefConvFCBBoxHead):
    """Shared2FC RBBox head."""

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(RotatedTeefShared2FCBBoxHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
...
@ROTATED_HEADS.register_module()
class RotatedTeefKFIoUShared2FCBBoxHead(RotatedTeefConvFCBBoxHead):
    """KFIoU RoI head."""

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(RotatedTeefKFIoUShared2FCBBoxHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
