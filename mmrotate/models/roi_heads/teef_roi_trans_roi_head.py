...
from mmrotate.core import (build_assigner, build_sampler, obb2xyxy,
                           rbbox2result, rbbox2roi, edl_rbbox2result)
from ..builder import ROTATED_HEADS, build_head, build_roi_extractor


@ROTATED_HEADS.register_module()
class TeefRoITransRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        super(TeefRoITransRoIHead, self).__init__(init_cfg)
    ...
    def _bbox_forward(self, stage, x, ted3_x, rois):
        ...
        bbox_feats = bbox_roi_extractor(x[:bbox_roi_extractor.num_inputs],
                                        rois)
        cls_score, bbox_pred = bbox_head.forward(bbox_feats)
        e_sum = bbox_head.e_sum_forward(bbox_feats)

        # ted3_results
        ted3_bbox_feats = bbox_roi_extractor(ted3_x[:bbox_roi_extractor.num_inputs],
                                             rois)
        ted3_cls_score, ted3_bbox_pred = bbox_head.ted3_forward(ted3_bbox_feats)
        ted3_e_sum = bbox_head.ted3_e_sum_forward(ted3_bbox_feats)

        p_e = F.softmax(cls_score, dim=1)
        e_sum = torch.exp(e_sum)
        ted3_p_e = F.softmax(ted3_cls_score, dim=1)
        ted3_e_sum = torch.exp(ted3_e_sum)
        e = e_sum * p_e
        ...
        bbox_results = dict(
            cls_score=p_e, e_sum=e_sum, bbox_pred=ted3_bbox_pred)
        return bbox_results

    def _bbox_forward_train(self, stage, x, ted3_x, sampling_results, gt_bboxes,
        ...
        bbox_results = self._bbox_forward(stage, x, ted3_x, rois)
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            bbox_results = self._bbox_forward_train(i, x, ted3_x, sampling_results,
                                                    gt_bboxes, gt_labels,
    ...
    def simple_test(self, x, ted3_x, proposal_list, img_metas, rescale=False):
        ...
        for i in range(self.num_stages):
            bbox_results = self._bbox_forward(i, x, ted3_x, rois)
            ...
            # u
            e_sum = bbox_results['e_sum']
            K = torch.tensor(cls_score.shape[1])
            u = K / (K + e_sum)
        ...
        u = [
            sum([tmp_u[i] for tmp_u in ms_us]) / float(len(ms_us))
            for i in range(num_imgs)
        ]
        ...
        # ]
        bbox_results = [
            edl_rbbox2result(det_bboxes[i], det_labels[i],
                             self.bbox_head[-1].num_classes)
                             for i in range(num_imgs)
        ]
