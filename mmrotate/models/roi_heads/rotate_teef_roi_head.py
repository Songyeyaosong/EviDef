...
from mmrotate.core import build_assigner, build_sampler, obb2xyxy, rbbox2result, edl_rbbox2result
from ..builder import (ROTATED_HEADS, build_head, build_roi_extractor,
                       build_shared_head)


@ROTATED_HEADS.register_module()
class RotatedTeefRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        super(RotatedTeefRoIHead, self).__init__(init_cfg)
    ...
    def _bbox_forward(self, x, rois):
        ...
        e_sum = self.bbox_head.e_sum_forward(bbox_feats)

        bbox_results = dict(
            cls_score=cls_score, e_sum=e_sum, bbox_pred=bbox_pred, bbox_feats=bbox_feats)
        return bbox_results
    
    def _ted3_bbox_forward(self, x, rois):
        """Box head forward function used in both training and testing.
        ...
        if self.with_shared_head:
            bbox_feats = self.shared_head(bbox_feats)
        cls_score, bbox_pred = self.bbox_head.ted3_forward(bbox_feats)
        e_sum = self.bbox_head.ted3_e_sum_forward(bbox_feats)

        bbox_results = dict(
            ted3_cls_score=cls_score, ted3_e_sum=e_sum, ted3_bbox_pred=bbox_pred, ted3_bbox_feats=bbox_feats)
        return bbox_results
    ...
    def simple_test(self, x, ted3_x, proposal_list, img_metas, rescale=False):
        ...
        det_bboxes, det_labels = self.simple_test_bboxes(
            x, ted3_x, img_metas, proposal_list, self.test_cfg, rescale=rescale)
