...
def imshow_det_rbboxes(img,
    ...
    if score_thr > 0:
        assert bboxes is not None and (bboxes.shape[1] == 6 or bboxes.shape[1] == 7)
        scores = bboxes[:, -1] if bboxes.shape[1] == 6 else bboxes[:, -2] # if shape[1]==6, last dim is score, otherwise is the u
    ...
    if bboxes is not None:
        ...
        scores = bboxes[:, 5] if bboxes.shape[1] == 6 or bboxes.shape[1] == 7 else None
        us = bboxes[:, 6] if bboxes.shape[1] == 7 else None
        draw_labels(
            ax,
            labels[:num_bboxes],
            positions,
